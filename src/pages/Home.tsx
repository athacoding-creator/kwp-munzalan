...
export default function Home() {
  ...
  const filteredArtikel = pengumumanTerbaru.filter(item => 
        ...
        <div className="w-full px-4 md:px-8 lg:px-12">
          ...
          <div className="max-w-7xl mx-auto">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {pengumumanTerbaru.map((item, index) => (
                <Card 
                  key={item.id} 
                  className="group bg-card border-0 shadow-sm hover:shadow-elegant transition-all duration-300 hover:-translate-y-1 rounded-xl overflow-hidden cursor-pointer animate-fade-in"
                  style={{ animationDelay: `${index * 100}ms` }}
                  onClick={() => setSelectedArtikel(item)}
                >
                  {/* Image */}
